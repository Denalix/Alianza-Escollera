...
//                                          D3    81    9A    01    22    09    36    05    16    55    55    7C    56    35    55    E8
static const PROGMEM u1_t NWKSKEY[16] = { 0xD3, 0x81, 0x9A, 0x01, 0x22, 0x09, 0x36, 0x05, 0x16, 0x55, 0x55, 0x7C, 0x56, 0x35, 0x55, 0xE8};

// LoRaWAN AppSKey, application session key
// This should also be in big-endian (aka msb).
//                                          0D    13    D8    79    B8    73    DE    7B    EB    AF    2A    63    D8    D7    E0    C8
static const u1_t PROGMEM APPSKEY[16] = { 0x0D, 0x13, 0xD8, 0x79, 0xB8, 0x73, 0xDE, 0x7B, 0xEB, 0xAF, 0x2A, 0x63, 0xD8, 0xD7, 0xE0, 0xC8};

// LoRaWAN end-device address (DevAddr)
// See http://thethingsnetwork.org/wiki/AddressSpace
// The library converts the address to network byte order as needed, so this should be in big-endian (aka msb) too.
//                          26 0D D2 58
static const u4_t DEVADDR = 0x260DD258; // <-- Change this address for every node!
...
void do_send(osjob_t* j) {
  ...
  if (LMIC.opmode & OP_TXRXPEND) {
    ...
    tempAverage = (tempAverageCond + tempAverageDO) / 2;
