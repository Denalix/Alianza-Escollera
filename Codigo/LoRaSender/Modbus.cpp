...
TRegister* Modbus::searchRegister(word address) {// ESTAMOS AQUI (3.7.2)
    TRegister *reg = _regs_head;
    //if there is no register configured, bail
    if(reg == 0) return(0);
    //scan through the linked list until the end of the list or the register is found. return the pointer
    //return the pointer.
    do {
...
word Modbus::Reg(word address) {// AHORA VAMOS ALLA O SEA AQUI(3.7.3.1)
    TRegister *reg;
    //search for the register address
    reg = this->searchRegister(address);
    //if found then return that register.
    if(reg)
        return(reg->value);
    else
        return(0);
...
//Use addHreg() for analog outpus or to store values in device 
...
word Modbus::Hreg(word offset) {// ESTAMOS AQUIIIIII (3.7.3)
    return Reg(offset + 40001);// AHORA VAMOS ALLA (3.7.3.1)
}
...
void Modbus::receivePDU(byte* frame) { // USADA EN MODBUSSERIAL (3.6)
    byte fcode  = frame[0];// DEBE REFERIRSE AL CÓDIGO DE FUNCION
    word field1 = (word)frame[1] << 8 | (word)frame[2];// DEBE TOMAR EL REGISTRO INICIAL (START ADDRESS)
    word field2 = (word)frame[3] << 8 | (word)frame[4];//DEBE TOMAR EL NUMERO DE REGISTROS (NUM REGISTERS)

    switch (fcode) {// MÁQUNA DE ESTADOS PARA EL CÓDIGO DE FUNCIÓN
        ...
        break;
        ///////////////////////////// LO QUE NOS INTERESA ////////////////////////////////////////
        case MB_FC_READ_REGS:// -> MB_FC_READ_REGS = 0x03, // Read Holding Registers 4xxxx
            //field1 = startreg, field2 = numregs
            //************** HASTA AQUI LA TRAMA VA: ID|FUNCION **************
            this->readRegisters(field1, field2);//VAMOS ACA (3.7)
        break;
        ////////////////////////////////////////////////////////////////////////////////////////////////////
...
void Modbus::readRegisters(word startreg, word numregs) {// ESTAMOS ACA (3.7.1)
    //Check value (numregs)
    if (numregs < 0x0001 || numregs > 0x007D) {// VERIFICAR QUE EL NUMREGS SEA MAYOR A 1 Y MENOR A 125
    ...
    // 0X40001 = 262145, tal vez le suma 4 al HIGH Y 1 al LOW
    if (!this->searchRegister(startreg + 40001)) {// VAMOS ALLÁ (3.7.2)
        //scan through the linked list until the end of the list or the register is found. return the pointer
        this->exceptionResponse(MB_FC_READ_REGS, MB_EX_ILLEGAL_ADDRESS);
        return; //O SEA QUE ESTAS LINEAS VERIFICAN SI 
    }
	...
	while(numregs--) {
        ...
        val = this->Hreg(startreg + i);//ESTAMOS AQUIIIIIIIIIII (3.7.3)
        //save the high byte of the register value into the frame
        _frame[2 + i * 2]  = val >> 8;
        //save the low byte of the register value into the frame
        _frame[3 + i * 2] = val & 0xFF;
        i++;
	}

    _reply = MB_REPLY_NORMAL;// AQUI REGRESAMOS A MODBUSSERIAL (3.7.4)
}
